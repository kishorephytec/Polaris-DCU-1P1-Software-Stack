...
TEMP_FILE="/var/www/html/rauc-os-update-config.tmp"
SETTINGS_FILE="/etc/rauc-hawkbit-updater/config.conf"

systemctl stop rauc-hawkbit.service
sleep 2
...
if [ -f "$TEMP_FILE" ]; then
    ...
    if [ "$HAWKBIT_SERVER" != "$TEMP_HAWKBIT_SERVER" ] || [ "$TARGET_NAME" != "$TEMP_TARGET_NAME" ] || [ "$AUTH_TOKEN" != "$TEMP_AUTH_TOKEN" ]; then
        ...
        echo "RAUC os update config settings updated successfully"
        systemctl status rauc-hawkbit-updater.service >> /var/log/rauc-hawkbit-os-update.log
    else
        echo "RAUC os update config settings are the same. No changes required."
        exit 0
    fi
else
    # Update temp file
